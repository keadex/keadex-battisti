...
var ScrollMagic = (function (window, document) {
  ...
  ScrollMagic.Scene = function (options) {
    ...
    var onMousewheelOverPin = function onMousewheelOverPin(e) {
      if (_controller && _pin && _state === SCENE_STATE_DURING && !_controller.info("isDocument")) {
        ...
        //------ START JACK PATCH
        //Patch to handle also TOUCH scrolling (not only mouse wheel) if the scene is in fixed state. Without this patch touch scrolling does not work
        //over fixed scene and a container set (by default the cointainer is the body element - only with the body it works without the patch)
        var overflow = getComputedStyle(e.target).overflow;
        var overflowY = getComputedStyle(e.target).overflowY;
        if (_pin != e.target && overflow != undefined && (overflow == "auto" || overflow == "scroll" || overflowY == "auto" || overflowY == "scroll")){
          // e.preventDefault(); //prevent scrolling if touching a target with a pointer events property
          // e.stopPropagation();
          return;
        }
        if (e.changedTouches){
          var currentY = e.touches[0].clientY;
          var delta = 50; //default move up
          if(currentY > lastY){
            //move down
            delta *= -1;
          }
          lastY = currentY;
          _controller._setScrollPos(_controller.info("scrollPos") + delta);
        //------ END JACK PATCH
    ...
    this.setPin = function (element, settings) {
      ...
      _pin.addEventListener("touchmove", onMousewheelOverPin, { passive: false }); //JACK PATCH to handle also touch scrolling
    ...
    this.removePin = function (reset) {
      if (_pin) {
        ...
        _pin.removeEventListener("touchmove", onMousewheelOverPin);
  ...
  var _util = ScrollMagic._util = function (window) {
    ...
    U.addClass = function (elem, classname) {
      ...
      if (classname) {
        //------- START PATCH JACK
        //Patch to handle multiple css classes. elem.classList.add(classname) does not work with multiple css classes
        if (elem.classList) {
    ...
    U.removeClass = function (elem, classname) {
      ...
      // }
      //------- START PATCH JACK
      //Patch to handle multiple css classes. elem.classList.remove(classname) does not work with multiple css classes
      if (classname) {
        if (elem.classList) {
...
var SceneBase = function (_React$PureComponent) {
  ...
  createClass(SceneBase, [{
    ...
    value: function initEventHandlers() {
      ...
      //------ START JACK PATCH
      //Patch to pass in the children function always the latest event and the ScollMagic Scene object (target)
      var childFuncReturn = callChildFunction(children, 0, {type: 'init', target: this.scene});
      if (typeof children !== 'function' && !isGSAP(childFuncReturn)) {
        return;
      }
