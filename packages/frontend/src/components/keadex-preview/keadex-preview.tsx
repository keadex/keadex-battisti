...
export const KeadexPreview : React.FunctionComponent = React.memo((props) => {
  ...
  function startAnimation(){
    ...
    //set bubbles into svg paths (#targe1, #target2, #target3) 
    svgPathRef.forEach((value, index) =>{
  ...
  return (
    <React.Fragment>
      <div className={`${styles["keadex-preview__section"]} ${styles["keadex-preview__cover"]}`} id="keadex-preview-logo-container">
        <div className="full-center position-absolute text-center">
          <div className="position-relative d-inline-block">
          {
            getTechLabels()
          }
          <svg id="svg" viewBox="0 0 1145 205" height="60px" ref={svgRef} className={`${styles["keadex-preview__logo"]}`} onClick={(e)=>{e.preventDefault(); startAnimation();}}>
            <g id="DEX" style={{opacity: "0"}}>
              <text className={`${styles["cls-1"]}`} transform="translate(872.4 191.51) scale(1.75 1)">DE</text>
              <text className={`${styles["cls-2"]}`} transform="translate(979.69 174.72) scale(2.85 1)">V</text>
              <text className={`${styles["cls-2"]}`} transform="translate(1032.13 175.37) rotate(180) scale(2.85 1)">V</text>
            </g>
