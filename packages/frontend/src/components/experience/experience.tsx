...
class Experience extends React.Component<ExperienceProps, ExperienceState> {
  ...
  private logo : any = {
    "atos-logo.png": "logo/atos-logo.png",
    "ibm-logo.png": "logo/ibm-logo.png",
    "open-reply-logo.png": "logo/open-reply-logo.png",
    "univ-liverpool-logo.png": "logo/univ-liverpool-logo.png",
    "vodafone-logo.png": "logo/vodafone-logo.png"
  }
                              ...
                              <Tween staggerFrom={{ opacity: 0, cycle: { x: (i:number) => (i+1) * 50 }}} stagger={0.1} playState={this.getPlayState(this.props.progress)}>
                                <div className={`${styles["experience__section-title"]} mb-md-3 mb-2`}>{"<"}<FormattedMessage id={"ABOUT_ME.EXPERIENCE.COMPANIES"} />{"/>"}</div>
                                {
                                  this.props.experience.companies.map((value, index) =>{
                                    return <div className="d-inline d-md-block mx-1 mx-md-0" key={index}><OptimizedImage src={this.logo[value.logoFilename]} className={`mb-3 mb-md-4 ${styles["experience__logo-brands"]} d-inline`} alt={`${value.name}`}/></div>
                                  })
                                }
                              </Tween>
                            </div>
