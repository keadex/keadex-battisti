...
import HeaderSoftwareEngineer from '../header-software-engineer/HeaderSoftwareEngineer';
import HeaderArchitect from '../header-architect/HeaderArchitect';
import HeaderMobileDeveloper from '../header-mobile-developer/HeaderMobileDeveloper';
      ...
      <React.Fragment>
        ...
        { this.props.experience != undefined && (
          <React.Fragment>
            ...
            {/* CONTENT */}
            <Tween            
              from={{css: {className: "animate__animated animate__fadeOut experience__body"}}}
              to={{css: {className: "animate__animated animate__fadeIn experience__body"}}}
              progress={progress}
              playState="stop">            
              <div className="experience__body" onMouseOver={()=>{this.stopSlideshow()}} onMouseOut={()=>{this.startSlideshow()}} onTouchStart={()=>{this.stopSlideshow()}} onTouchEnd={()=>{this.startSlideshow()}}>
              ...
              <MDBCarousel mobileGesture={false} activeItem={1} length={2} interval={10000000} className="z-depth-1 h-100" ref={this.refCarousel} >
                ...
                <MDBCarouselInner>
                  <MDBCarouselItem itemId="1">
                    <MDBView className="h-100 w-100">
                      <MDBCard className="h-100 w-100">
                        <MDBCardBody className="experience__card-body">
                          
                          {/* POSITION */}                          
                          <MDBCardTitle className="text-center">
                            <div className='avatar d-inline-block experience__avatar'>
                              <img src={require(`../../assets/img/jack/${this.props.experience.avatar_filename}`)} alt="avatar position"/>
                            </div>
                            <div className="experience__title-position text-left text-md-center">
                          ...
                          <div className="experience__card-content-container">
                            {/* TASKS */}
                            ...
                            <div className={"col-12 col-md-4 p-0 pl-md-2 pr-md-2 text-center order-3 order-md-2"}>
                              <Tween staggerFrom={{ opacity: 0, cycle: { x: (i:number) => (i+1) * 50 }}} stagger={0.1} playState={this.getPlayState(this.props.progress)}>
                                <div className="experience__section-title mb-md-3 mb-2">{"<"}<FormattedMessage id={"ABOUT_ME.EXPERIENCE.COMPANIES"} />{"/>"}</div>
                                {
                                  this.props.experience.companies.map((value, index) =>{
                                    return <div className="d-inline d-md-block mx-1 mx-md-0" key={index}><img src={require(`../../assets/img/logo/${value.logo_filename}`)} className="mb-3 mb-md-4 experience__logo-brands d-inline" alt={`${value.name}`}/></div>
                                  })
                                }
                              </Tween>
                            </div>
                                ...
                                <Tween staggerFrom={{ opacity: 0, cycle: { x: (i:number) => (i+1) * 50 }}} stagger={0.1} playState={this.getPlayState(this.props.progress)}>
                                  <div className="experience__section-title mb-md-3 mb-2">{"<"}<FormattedMessage id={"ABOUT_ME.EXPERIENCE.CUSTOMERS"} />{"/>"}</div>
                                  {
                                    this.props.experience.customers.map((value, index) =>{
                                      return <div className="w-100" key={index}><img src={require(`../../assets/img/logo/${value.logo_filename}`)} className="mb-3 mb-md-4 experience__logo-brands" alt={`${value.name}`}/></div>
                                    })
                                  }
                                </Tween>
                              </div>
                  ...
                  <MDBCarouselItem itemId="2">
                    <MDBView className="h-100 w-100">
                      <MDBCard className="h-100 w-100">
                        <MDBCardBody className="pt-5 h-100 experience__card-body-chart">
                          <div className="experience__skills-chart-container pointer-events-all">
                            <ResponsiveContainer>
                              <PieChart>
                            ...
                            {
                              <div className="row justify-content-center mt-2 mt-md-5 experience__legend-container pointer-events-all">
                                {
                                  this.props.experience.skills.map((entry, index) => {
                                    return (
                                    <div className="col-auto experience__legend" key={index}>
